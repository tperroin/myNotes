...
@section('navigation')
    ...
    @section('content')
        ...
        <h1>Créer un compte utilisateur pour l'élève {{$student->lastname}} {{$student->firstname}}</h1>

        <!-- if there are creation errors, they will show here -->
        {{ HTML::ul($errors->all()) }}

        {{ Form::open(array('url' => '/admin/students/store/user', 'class' => 'ui form')) }}

                {{ Form::label('lastname', 'Nom') }}
                {{ Form::text('lastname', $student->firstname, Input::old('lastname')) }}

                {{ Form::label('firstname', 'Prénom') }}
                {{ Form::text('firstname', $student->lastname, Input::old('firstname')) }}

                {{ Form::label('email', "Email") }}
                {{ Form::email('email', $student->email, Input::old('email')) }}
                ...
                {{ Form::hidden('id', $student->id, Input::old('id')) }}
